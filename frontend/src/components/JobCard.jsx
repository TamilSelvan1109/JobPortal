...
const JobCard = ({job}) => {
  ...
  return (
    <div className='border p-6 shadow rounded'>
      ...
      <p className='text-gray-600 text-sm mt-4 line-clamp-3' dangerouslySetInnerHTML={{__html:job.description.slice(0,200)}}></p>
      <div className='mt-4 flex gap-4 text-sm'>
        ...
        <button onClick={()=>{navigate(`/apply-job/${job._id}`); scrollTo(0,0)}} className='text-gray-600 border border-gray-600 rounded px-4 py-2 cursor-pointer'>Learn More</button>
      </div>
    </div>
  )
}
